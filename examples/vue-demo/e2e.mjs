// End-to-end verification of the full AgentLens chain on a Vue 3 app:
//   browser signals -> runtime SDK -> WebSocket -> daemon -> MCP tools.
// Usage: pnpm --filter vue-demo e2e
...
// dev server, the other demos' E2E runs or a Cursor-managed daemon (8631).
...
async function main() {
  ...
  try {
    ...
    // Source attribution: the vite plugin must tag template elements with
    // the original source location before @vitejs/plugin-vue compiles them.
    const sourceAttr = await page.getAttribute('#btn-error', 'data-agentlens-source');
    assert(
      /^src\/App\.vue:\d+$/.test(sourceAttr ?? ''),
    ...
    assert(
      errors.some((e) => e.frames.some((f) => f.fileName?.includes('App.vue') && f.line > 0)),
      'sourcemap: error stack resolved to original source (App.vue)',
    );
    assert(
      network.some((e) => e.initiatorFrames.some((f) => f.fileName?.includes('App.vue'))),
    ...
    const buttonNode = findNode(
      snapshot.root,
      (n) => n.tag === 'button' && /^src\/App\.vue:\d+$/.test(n.source ?? ''),
    );
    assert(
    ...
    assert(
      errorGroup !== undefined &&
        /^src\/App\.vue:\d+$/.test(errorGroup.interaction.target.source ?? ''),
    ...
    // Positive path: touch App.vue to trigger a real HMR update while
    // verify_fix is waiting; the error does not recur, so it verifies.
    // The comment must land inside the template block — root-level SFC
    // comments belong to no block, so plugin-vue treats them as no-ops.
    ...
    await writeFile(
      APP_FILE,
      originalAppSource.replace('</template>', '<!-- e2e-hmr-touch --></template>'),
      'utf8',
    );
  ...
  console.log('\nE2E PASSED: full chain verified (Vue browser -> runtime -> daemon -> MCP)');
